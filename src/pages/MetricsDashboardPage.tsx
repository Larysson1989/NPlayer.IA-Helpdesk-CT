                ...
                <div className="space-y-8">
                  ...
                  <section>
                    ...
                    <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-6 gap-3">
                      <KpiCard label="Usuários ativos" value={kpis?.usuariosAtivos ?? 0} icon={<Users size={18} />} color="blue" />
                      <KpiCard label="Logins totais" value={kpis?.totalLogins ?? 0} icon={<LogIn size={18} />} color="indigo" />
                      <KpiCard label="Mensagens enviadas" value={kpis?.totalMensagens ?? 0} icon={<MessageSquare size={18} />} color="violet" />
                      <KpiCard label="Correções à IA" value={kpis?.totalCorrecoes ?? 0} icon={<Repeat2 size={18} />} color="amber" alert={(kpis?.totalCorrecoes ?? 0) > 50} />
                      <KpiCard label="Taxa de acerto IA" value={`${kpis?.taxaAcertoIA ?? 0}%`} icon={<Target size={18} />} color={(kpis?.taxaAcertoIA ?? 0) >= 90 ? 'emerald' : (kpis?.taxaAcertoIA ?? 0) >= 70 ? 'amber' : 'red'} />
                      <KpiCard label="Usuários inativos" value={kpis?.usuariosInativos3d ?? 0} icon={<UserX size={18} />} color="slate" alert={(kpis?.usuariosInativos3d ?? 0) > 0} />
                    </div>
                  </section>
                  ...
                  <section>
                    ...
                    <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                      <KpiCard label="Logins" value={kpis?.logins7d ?? 0} icon={<LogIn size={18} />} color="sky" delta="vs. período anterior" />
                      <KpiCard label="Mensagens" value={kpis?.mensagens7d ?? 0} icon={<MessageSquare size={18} />} color="purple" />
                      <KpiCard label="Usuários únicos" value={kpis?.usuariosAtivos ?? 0} icon={<Users size={18} />} color="teal" />
                      <KpiCard label="Correções" value={kpis?.totalCorrecoes ?? 0} icon={<ThumbsDown size={18} />} color="rose" alert={(kpis?.totalCorrecoes ?? 0) > 10} />
                    </div>
                  </section>
                ...
                <div className="space-y-8">
                  <section>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      <IaAccuracyGauge taxa={kpis?.taxaAcertoIA ?? 0} />
                      <div className="grid grid-cols-2 gap-3">
                        <KpiCard label="Total de mensagens" value={kpis?.totalMensagens ?? 0} icon={<MessageSquare size={18} />} color="blue" />
                        <KpiCard label="Correções totais" value={kpis?.totalCorrecoes ?? 0} icon={<Repeat2 size={18} />} color="amber" />
                        <KpiCard label="Correções (7d)" value={kpis?.totalCorrecoes ?? 0} icon={<AlertTriangle size={18} />} color="rose" alert={(kpis?.totalCorrecoes ?? 0) > 10} />
                        <KpiCard label="Taxa acerto (7d)" value={`${kpis?.taxaAcertoIA ?? 0}%`} icon={<Zap size={18} />} color="emerald" />
                      </div>
                    </div>
                  </section>
